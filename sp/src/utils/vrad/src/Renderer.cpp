...
void Renderer::RenderSkyDome(Vector3f sunDir, const char *filename, unsigned width, unsigned height, float sunSize, Vector3f sunColor, Vector3f groundAlbedo, float turbidity, Vector3f skyTint, Vector3f sunTint, bool bEnableSun, ColorSpace colorspace)
{
	SkyModel sky;
	sky.SetupSky(sunDir,
		sunSize,
	...
	for (unsigned j = 0; j < height; ++j) {
		...
		for (unsigned i = 0; i < width; ++i, ++p) {
			...
			if (z2 <= 1) {
				...
				*p = sky.Sample(dir, bEnableSun, skyTint, sunTint);
			}
		}
	}
#endif
